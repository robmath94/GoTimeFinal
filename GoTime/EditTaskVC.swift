...
class EditTaskVC: UIViewController, UITextViewDelegate, UITextFieldDelegate, UIPickerViewDelegate, UIPickerViewDataSource, MyProtocol {
   ...
   var taskNameHolder:String = ""
   var priorityControlHolder:Int = 0
   var dueDateHolder:Date = Date()
   var groupLabelHolder:String = ""
   var reminderHolder:Int = 0
   ...
   override func viewDidLoad() {
      ...
      groupButton.setTitle(groupLabelHolder, for: .normal)
      priorityControl.selectedSegmentIndex = priorityControlHolder
      dueDate.setDate(dueDateHolder, animated: false)
   ...
   @IBAction func editTaskButtonPressed(_ sender: UIButton) {
      if(!(taskName.text?.isEmpty)!) {
         let context = (UIApplication.shared.delegate as! AppDelegate).persistentContainer.viewContext
         
         taskToEdit.setValue(taskName.text, forKey: "name")
         taskToEdit.setValue(Double(priorityControl.selectedSegmentIndex), forKey: "priority")
         taskToEdit.setValue(groupButton.currentTitle, forKey: "group")
         ...
         taskToEdit.setValue(formatter.date(from: dueDateString)! as NSDate, forKey: "dueDate")
